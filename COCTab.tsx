...
interface BulkQuoteImportProps {
  onImport: (items: QuoteItem[]) => void;
  onCancel: () => void;
  isNewSupply?: boolean;
}

const BulkQuoteImport: React.FC<BulkQuoteImportProps> = ({ onImport, onCancel, isNewSupply }) => {
  ...
  const downloadTemplate = () => {
    ...
    const sampleData = [
      ['maintenance', 'Annual Service: Fire Extinguisher', '9kg', 5],
      ['replacement', 'New Fire Extinguisher', '4.5kg', 2],
      ['signage', 'Fire Extinguisher (FB1)', '190x190', 10],
      ['other', 'Fire Extinguisher Cabinet', '9kg', 1]
    ];

    const ws = XLSX.utils.aoa_to_sheet([headers, ...sampleData]);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Quote Items Template');
    XLSX.writeFile(wb, 'Quote_Items_Import_Template.xlsx');
  };
  ...
  const handleImport = () => {
    ...
    try {
      const items = previewData.map(mapToQuoteItem);
      onImport(items);
    } catch (err: any) {
    ...
    <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-xl z-[100] flex items-center justify-center p-4">
      <div className="bg-white w-full max-w-4xl rounded-[3rem] shadow-2xl border border-slate-200 overflow-hidden flex flex-col max-h-[90vh]">
        <div className="bg-slate-900 p-8 text-white flex justify-between items-center">
          <div>
            <h2 className="text-2xl font-black uppercase tracking-tight">Bulk Quote Item Import</h2>
            <p className="text-slate-400 text-xs mt-1">Upload a list of equipment to be quoted on.</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-4">
              ...
              >
                ...
                <span className="uppercase tracking-widest text-xs">Download Quote Template</span>
              </button>
            </div>
          ...
          {previewData.length > 0 && (
            <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4">
              <div className="flex justify-between items-center px-1">
                ...
                <span className="text-[10px] font-black text-green-600 uppercase tracking-widest bg-green-50 px-3 py-1 rounded-full">Ready to Add</span>
              </div>
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {previewData.slice(0, 5).map((row, idx) => (
                      <tr key={idx} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-6 py-4 font-black text-slate-600 text-xs uppercase">{row['Description'] || row['description'] || 'N/A'}</td>
                        <td className="px-6 py-4 font-black text-slate-600 text-xs uppercase">{row['Size'] || row['size'] || 'N/A'}</td>
                        <td className="px-6 py-4 font-black text-slate-600 text-xs uppercase">{row['Quantity'] || row['quantity'] || '1'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          >
            {isProcessing ? (
              ...
              <>
                ...
                <span>Add {previewData.length} Items to Quote</span>
              </>
            )}
          </button>
        </div>
